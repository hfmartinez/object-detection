...
def get_boxes(db: Session, image_id: int, label: str = None, confidence: float = 0.5):
    if not label:
        return (
            db.query(models.Box)
            .filter(models.Box.owner_id == image_id)
            .filter(models.Box.confidence >= confidence)
            .all()
        )
